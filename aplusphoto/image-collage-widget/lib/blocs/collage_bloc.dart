...
class CollageBloc extends Bloc<CollageEvent, CollageState> {
  ...
  openPicker(PermissionType permissionType, int index) async {
    ...
    if (image != null) {
      ...
      imageGlobal[index].imageUrl = File(image.path);
      add(ImageListEvent(imageGlobal));
    }
  }
  ...
  getImageCount() {
    if (collageType == CollageType.HSplit ||
      ...
      return 4;
    } else if (collageType == CollageType.ThreeVerticalLeft ||
        collageType == CollageType.ThreeVerticalRight ||
        collageType == CollageType.ThreeHorizontalTop ||
        collageType == CollageType.ThreeHorizontalBottom) {
      return 3;
    }
  }
